...
def printWhenExecuting_4_args(fn, arg1, arg2, arg3):
    def fn2(self, arg1, arg2, arg3):
        print("   doing", fn.__name__)
...
class TestApp(TestWrapper, TestClient):
    def __init__(self, data_queue):
        ...
        self.data_out_q = data_queue

        # User added class variables
    ...
    def start(self):
        ...
        if self.globalCancelOnly:
            ...
            print("Executing requests ... if any....finished")
    ...
    def historicalDataOperations_req(self, contract_desc, durationStr, barSizeSetting):
        """
        contract_desc - eg, ContractSamples.USStock("AAPL")
        ...
        #contract_desc = ContractSamples.USStock("AAPL")
        # Requesting historical data
        # ! [reqHeadTimeStamp]
        self.reqHeadTimeStamp(4101, contract_desc, "TRADES", 0, 1)
        # ! [reqHeadTimeStamp]
        ...
        # self.reqHistoricalData(4102, contract_desc, queryTime,
        #                         "2 D", "15 mins", "TRADES", 1, 1, False, [])

        # See ContractSamples above
        print("historicalDataOperations_req: Sending reqHistoricalData")
        self.reqHistoricalData(4102, contract_desc, queryTime,
                              durationStr, barSizeSetting, "TRADES", 1, 1, False, [])
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        self.data_out_q.put("HistoricalDataEnd")
        self.data_out_q.put(reqId)
        self.data_out_q.put(start)
        self.data_out_q.put(end)
        # self.historical_data_end_hook_func(reqId, start, end)
    ...
    def convert_bar_data_to_dict(self, bar: BarData):
        bar_dict = {'Date': bar.date,
                    'Open': bar.open,
                    'High': bar.high,
                    'Low': bar.low,
                    'Close': bar.close,
    ...
    def historical_data_hook_func(self, reqId, bar):
        print("*****historical_data_hook_func")
        logger.debug("historical")
        row_dict = self.convert_bar_data_to_dict(bar)
        new_data_df = pd.DataFrame(row_dict)
        self.historical_data_df.append(new_data_df, ignore_index=True)

    def historical_data_end_hook_func(self, reqId, start, end):
        print(self.historical_data_df)


class InteractiveConsoleThread(threading.Thread):
    def __init__(self, app, rx, tx, q):
...
TXState = Enum("IDLE_WAIT",
               "REQ_HISTORICAL_DATA",
               "DONE")


class TXThread(threading.Thread):
    def __init__(self, app, send_to_tx_queue):
        threading.Thread.__init__(self)
        self.args = []
        self.request = ""
        print_thread("TXThread: __init__")
        self.my_app = app
        self.req_state = TXState.IDLE_WAIT
        self.running = True
        self.send_to_tx_queue = send_to_tx_queue
        self.hist_data_cb = hist_data_cb

    def send_command(self, request, args: list):
        self.args = args
        self.request = request

    def get_state(self):
        print_thread("TXThread state = ", self.req_state)

    def terminate(self):
        self.running = False

    def run(self):
        print_thread("TXThread: run")

        # State machine for the thread state
        while self.running:
            time.sleep(0.1)
            # print("TX state = ", self.req_state)
            if self.req_state == TXState.IDLE_WAIT:
                if self.request != "":
                    self.req_state = self.request
                    self.request = ""
            if self.req_state == TXState.REQ_HISTORICAL_DATA:
                print(threading.get_ident(), "TXThread: REQ_HISTORICAL_DATA: args:")
                print(self.args[0], self.args[1], self.args[2])

                self.my_app.historicalDataOperations_req(self.args[0], self.args[1], self.args[2])
                #self.my_app.historicalDataOperations_req()
                # Wait and spin for historical data end signal from RXThread
                # while not self.histdata_done_event.is_set():
                print_thread("TXThread: wait for RX event")
                while True:
                    data = self.send_to_tx_queue.get()
                    if self.send_to_tx_queue.qsize() != 0:
                        print_thread("TXThread: receive done even from RX or timeout")
                        if data == "HistoricalDataEnd":
                            # Data frame is done by RX, process it
                            print_thread("TXThread: Data frame received... printing it:")
                            data = self.send_to_tx_queue.get()
                            print_thread(data)
...
class RXThread(threading.Thread):
    """
    Receiver (RX) thread to process queue filled by App and notify Transmitter (TX) thread if request is done
    """

    def __init__(self, app, queue, tx_queue):
        threading.Thread.__init__(self)
        ...
        self.data_list = []
        self.running = True
        self.send_to_tx_queue = tx_queue
    ...
    def convert_bar_data_to_list(self, bar: BarData):
        bar_list = [bar.date, bar.open, bar.high, bar.low, bar.close, bar.volume, bar.average, bar.barCount]
        return bar_list
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.in_q.qsize() != 0:
                #print_thread("RX receiving historical data")

                # print(data)
                if data == "HistoricalDataEnd":
                    print_thread("RXThread: !!!!  END OF HISTORICAL DATA !!!")
                    print_thread("RXThread: Received and completed data frame:")
                    self.historical_data_df = pd.DataFrame(self.data_list, columns=['Date', 'Open', 'High', 'Low',
                                                                                    'Close', 'Volume', 'Average',
                                                                                    'BarCount'])
                    print_thread(self.historical_data_df)
                    # signal TXThread that it is done
                    self.send_to_tx_queue.put("HistoricalDataEnd")
                    self.send_to_tx_queue.put(self.historical_data_df)

                else:
                    if isinstance(data, BarData):
                        row_list = self.convert_bar_data_to_list(data)
                        self.data_list.append(row_list)
                    else:
                        print(data)

            # time.sleep(0.1)
        print("RX thread stops....")


def print_thread(log: str):
    print("Thread #", threading.get_ident(), log)
...
def main():
    ...
    try:
        ...
        # ! [connect]
        # connect also launch eReader thread
        print("serverVersion:%s connectionTime:%s" % (app.serverVersion(),
                                                      app.twsConnectionTime()))

        tx_thread = TXThread(app, hist_q)
        tx_thread.start()

        tx_cmd_args = [ContractSamples.USStock("AAPL"), "2 D", "15 mins"]
        tx_thread.send_command(TXState.REQ_HISTORICAL_DATA, tx_cmd_args)
        rx_thread = RXThread(app, q, hist_q)
        rx_thread.start()

        ic = InteractiveConsoleThread(app, rx_thread, tx_thread, q)
        ic.start()
        print("***app run**")
...
def callback_req_historical_data(data):
    print_thread("callback_req_historical_data = ")
    print(data)
